...
class TextClassificationResponse {
  late TextClassificationResult? result; // Result object
...
// Function to create a response object from JSON data
  TextClassificationResponse.fromJson(Map<String, dynamic> json) {
    result = TextClassificationResult.fromJson(json['result']); // Create a result object from the JSON data
...
class TextClassificationResult {
  late double positive; // Positive score
  late double negative; // Negative score

  TextClassificationResult({required this.positive, required this.negative}); // Constructor
// Function to create a result object from JSON data
  TextClassificationResult.fromJson(List data) {
    negative =
        data.firstWhere((label) => label['label'] == "NEGATIVE")['score']; // Set the negative score
    positive =
        data.firstWhere((label) => label['label'] == "POSITIVE")['score']; // Set the positive score
  }
// Function to convert the result object to JSON data
  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = <String, dynamic>{}; // Create a map to store the data
    data['positive'] = positive; // Add the positive score to the map
    data['negative'] = negative; // Add the negative score to the map
    return data; // Return the map
  }
}
