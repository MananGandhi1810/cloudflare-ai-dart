...
// Constructor
  ...
  Future<Uint8List> generateImage(String prompt) async {
    if (prompt.trim().isEmpty) {
      throw Exception("Prompt cannot be empty"); // Throw an exception if prompt is empty
    }
    Map<String, dynamic> res = await networkService.post(
        ...
        isImage: true); // Post request to the API
    try {
      Uint8List result = res['data']; // Get the image data
      return result; // Return the image data
    } catch (e) {
      print(e); // Print the error
      throw Exception(res["errors"]); // Throw an exception with the error message
    }
  }
}
